...
async function main() {
    const [discover, trendingShowsObj, genresObject, genresShowsObject] = await Promise.all([
    ...
    for (let i = 0; i < trendingNow.length; i++) {
        ...
        let movieImgCount = 0;
        let movieImg = await fetch(`${TMDB_API_URL}/${trendingNow[i].media_type}/${trendingNow[i].id}/images`, options).then((response) => response.json());
...
function openGenre(event) {
    ...
    site = '/pages/movie-genre.html?id=' + genreId + `&type=${event.target.dataset.genreType}` + '&genre=' + genreName;
    window.location.href = site;
}

async function showMovies(genre, index, element, genreId, type) {
    let results = await fetch(`${TMDB_API_URL}/discover/${type}?sort_by=vote_count.desc&with_genres=${genreId}`, options)
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        let movieImgCount = 0;
    let movieImg = await fetch(`${TMDB_API_URL}/${type}/${results[i].id}/images`, options).then((response) => response.json());
        for (let i = 0; i < movieImg.backdrops.length; i++) {
            if (movieImg.backdrops[i].iso_639_1 == 'en' && movieImgCount == 0 && movieImg.backdrops.length > 0) {
                movieImgCount = 1;
                movieImgPath = movieImg.backdrops[i].file_path;
